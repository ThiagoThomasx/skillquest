...
import { Sword, Clock, Zap, Filter } from "lucide-react";
...
const missions = [
  ...
  { id: 3, title: "Context API vs Zustand", path: "Frontend", xp: 250, time: "60 min", progress: 0, status: "available", difficulty: "Difícil" },
...
const difficultyBadge: Record<string, "violet" | "gold" | "rose"> = {
  "Fácil": "violet",
  "Médio": "gold",
  "Difícil": "rose",
};
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-text">Missões</h2>
          <p className="text-text-muted mt-1">5 missões — 2 completas, 1 em andamento</p>
        </div>
            ...
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl shrink-0 flex items-center justify-center border ${
                ...
                <Sword size={16} className={
                  m.status === "completed" ? "text-emerald" : m.status === "active" ? "text-violet" : "text-text-dim"
                } />
              </div>
              ...
              <div className="flex-1 min-w-0 space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {m.status === "active" && <Badge variant="violet">Em andamento</Badge>}
                </div>
                <div className="flex items-center gap-3 text-xs text-text-muted">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gold">{m.xp} XP</span>
                  </div>
                </div>
                {m.status === "active" && (
                  <ProgressBar value={m.progress} variant="violet" size="sm" showLabel />
                )}
              </div>
